    ...
    <div class="d-flex flex-column align-content-center flex-wrap">
        ...
        <div class="w-90" v-if="!loading">
            <div v-for="(user, index ) in filteredUsers" v-auto-animate>
                <SingClassMem :unattended='user["unAttended_times"]' :key="user['id']" :name="user['name']"  :attendenceDates= "[user['date']] || []" :lastElement= "usersLen - 1 == index " :attendedArr="attendedUsersNames" />
            </div>
        </div>
    </div>
...
const AuthStore = AuthUser();
    ...
    methods: {
        async fetchAttendence() {
            ...
            const res = await axios.get(`/admin/app-sig-action?date=${date}&action=${action}`)
